...
defines the contribution of its kinematic chain to the rover body motion. 
Considering a single contact angle at each wheel and disregarding slip 
($\boldsymbol{\dot{\varepsilon}_{i}} = [\dot{\delta}_{i}]$), the size of the 
$J_{i}$ matrix is $6 \times (n + 1)$ where $n$ corresponds to the DoF of the
mechanism.
